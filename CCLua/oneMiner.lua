...
function main_run_test()
	bore(bore_points[tnum][1],bore_points[tnum][2],5)
	--bore(7,7) --boring close to the center of the chunk is safer
	chunk_step(0)
	main_run()
end
...
function start(arg_tnum)
	...
	if startcode == 1 then --real start
		--set startup to recovery
		dir_set()
		start_file = fs.open("startup","w")
		if start_file then
		...
		main_run()
	elseif startcode == 2 then --teststart
		--set startup to recovery
		dir_set()
		start_file = fs.open("startup","w")
		if start_file then
...
function group_step()
	...
	if tnum > 1 then
		...
		mmove(0,5)
		sleep(100) -- wait in case the lead is behind
	elseif tnum == 1 then
	--if lead turtle
