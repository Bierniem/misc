...
function move_to(newpos)
	...
	while not (pos[1] == newpos[1] and pos[2] == newpos[2] and pos[3]==newpos[3]) do --try until success
		...
		elseif unsticker%2 == 0 then --only every other so it doesn't prevent the while from succeeding
			mmove(4,unsticker)
		end
...
function dump_stuff()
	race_writer("2,0")
	while not place(0,chest_slot) do
		turn(1)
	end
	...
	turtle.select(chest_slot)--clear the chest slot
	turtle.drop()
...
function build_loader()
	...
	place(0,loader_slot)
	if not isOre(0,anchor_block) then
		print "failed build loader"
		full_stop("failed build loader")
	end
...
function destroy_loader()
	if not isOre(0,anchor_block) then
		print "failed destroy loader"
		full_stop("failed destroy loader")
	end
...
function recover_loader()
	if (turtle.getItemCount(loader_slot) == 0 or turtle.getItemCount(rtg_slot) == 0) then
		failure_append("attempting recover_loader")
		--need to recover loader
		--check previous chunk
		move_to({8,0,0})
		turn(2-direction)
		if isOre(0,anchor_block) then
			destroy_loader()
			send_step() -- move everyone out of the previous chunk before unloading it
			failure_append("loader at location 8,0,0")
			return
		end
		--check edge of current chunk
		move_to({8,14,0})
		turn(0 - direction)
		if isOre(0,anchor_block) then
			destroy_loader()
			failure_append("loader at location 8,14,0")
			return
		end
		full_stop()
	end
...
function chunk_step()
	...
	recover_loader() -- if we've stopped in the middle of a chunk step handle this
	...
	if tnum == 1 then
		...
		move_to({0,15,0})
		turn(0-direction)
		send_step()--tell other turtles to move into new chunk
		--enter new chunk
		mmove(0,5)
		move_to({8,0,0})
		turn(2-direction)
		...
		destroy_loader()
		--move away from the loader for safety
	end
	step_reset()
end

function first_chunk()
	--place loader at edge of current chunk
	--should start at 0,0
	move_to({7,1,0})
	turn(2 - direction)
	if not build_loader() then
		print("failed to build loader")
		exit()
	end
end
...
function recovery_startup()
	...
	--if racefile is empty, dir and pos are accurate
		...
		chunk_step()
		main_run()
	elseif races[1] == 2 then
		--pickup your chest
		dig(0, chest_slot, false)
	elseif races[1] == 1 then
	--if racefile includes "turn" pos is accurate but dir is not
		failure_append("dir race condition on restart")
		move_to({pos[1],pos[2],0})
		if dir_recovery(races[2]) then
			chunk_step()
			main_run()
		else 
			print("failed dir_recovery")
			full_stop("failed dir_recovery")
		end
...
function dir_set()
	...
	turn(0-direction)
	place(0,compass_slot)
	b,d = turtle.inspect()
	dig(0,compass_slot,false)
...
function main_run_test()
	while true do
		bore(bore_points[tnum][1],bore_points[tnum][2],10)
		--bore(7,7) --boring close to the center of the chunk is safer
		chunk_step()
	end
end

function test_chunkloader(tnum_arg)
	tnum = tonumber(tnum_arg)
	pos = {tnum-1,0,0}
	if tnum == 1 then
		first_chunk()
	end
	while true do
		bore(bore_points[tnum][1],bore_points[tnum][2],4)
		chunk_step()
	end
end

function test_recovery_startup()
	sleep(5)
	print("test_recovery_startup")
	step_reader()
	state_reader() -- get saved state
	dir_recovery()
	printStats()
...
function start(arg_tnum)
	tnum = tonumber(arg_tnum)
	pos = {tnum-1,0,0}
	if tnum == 1 then
		print("place me in position (0,0) in a chunk\nchest_slot=1, loader_slot=2, rtg_slot=3, turtle_slot=4, bucket_slot=5, coal_slot=6\nenter '1' to start test, enter '2' to start full run")
	elseif (tnum > 1) then
		print("place me in position (0,",tnum,") in a chunk\nchest_slot=1, junk=2, junk=3, turtle_slot=4, bucket_slot=5, coal_slot=6\nenter '1' to start test, enter '2' to start full run")
	else
		print("start invalid tnum argument ",arg_tnum)
		return false
	end
	startcode=io.read()
	startcode=tonumber(startcode)
	--if startcode==1 then print("try again!") exit() end
	if startcode == 1 then --test
		...
		if tnum == 1 then
			first_chunk() --setup initial chunk loader
		end
		main_run()
	elseif startcode == 2 then --actual start
		--set startup to recovery
		dir_set()
		start_file = fs.open("startup","w")
		if start_file then
...
function group_step()
	...
	--if lead turtle
		...
		print("invalid tnum in group_step ",tnum)
	end
end
